...
axios.get('../data/elements.json').then(function(response) {
    ...
    allElements.forEach(element => {
        if (localStorage.getItem("name") === element.name) {
            ...
            model.innerHTML = `<model-viewer src=${element.bohr_model_3d} poster=${element.bohr_model_image} ar environment-image=${element.bohr_model_image} camera-orbit="-45deg 30deg 9m" overflow box style="height:700px; width:100%;" autoplay shadow-intensity="1" camera-controls touch-action="pan-y"></model-viewer>`
        }
    });
});